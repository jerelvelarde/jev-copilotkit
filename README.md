...
Open `/tool-bench`, pick one live lookup, and press **Start the race**. Four CopilotKit agents receive the same run specification at the same moment and each one:
...
- Jev accepts at most 255 choices per request. For larger link sets, every link enters a bounded Choice group; the 32 highest-probability links from each group advance through further Choice rounds until a final choice is possible. At most three groups run concurrently. This avoids a separate long Noul question for every link. LLM baselines receive the full available set and return one validated index. The policies differ and are disclosed rather than presented as a controlled benchmark.
- Each lane shows model time, Wikipedia retrieval time, and elapsed time separately. The starting and destination pages load before the lanes start; large Wikipedia page responses and upstream rate limits can still dominate a race.
- Wikipedia retrieval uses one MediaWiki REST page-with-HTML response and is capped at 5,000 links per article. An oversized article fails explicitly; the app never silently drops the rest of its links. Transient rate limits and network timeouts receive bounded retries.
